...
public class MailService 
{
    ...
    public void sendMessageHTMLQuestionAddress(String to, String subject, String text) throws AddressException, MessagingException
    {
    	createProperties();
    	Session session = Session.getInstance(props, null);
    	Message msg = new MimeMessage(session);
    	Address[] replyToList = new Address[1];
    	replyToList[0] = new InternetAddress(question);
    	msg.setReplyTo(replyToList);
    	msg.setFrom(new InternetAddress(question));
    	msg.setRecipients(Message.RecipientType.TO,InternetAddress.parse(to, false));
    	msg.setRecipients(Message.RecipientType.BCC,InternetAddress.parse(bcc, false));
    	msg.setSubject(subject);
    	msg.setContent(text, "text/html; charset=utf-8");
